...
(function (root, factory) {
  ...
  /* TWO LEVELS, matching the two media boxes the editor already has:
   *   question — fills the question's own image/GIF/video box
   *   answers  — fills the image box on EVERY answer choice
   *
   * There is deliberately no "right answer only" setting. The editor has no box for it,
   * and writing a picture to the correct option alone marks that card out the moment
   * the choices are on screen.
   *
   * `answer` and `allAnswers` are the earlier spellings. Decks saved under them keep
   * working rather than silently losing their setting. */
  function gifPolicy(raw) {
    const r = (raw && typeof raw === 'object') ? raw : {};
    return {
      question: r.question === true,
      answers: r.answers === true || r.allAnswers === true || r.answer === true,
    };
  }
  const gifsEnabled = (raw) => { const p = gifPolicy(raw); return p.question || p.answers; };
