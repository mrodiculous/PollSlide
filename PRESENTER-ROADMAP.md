...
### Stage 3 — High grade (SHIPPED ✅ 2026-07-04)
...
Also shipped: per-element **build animations** (✦ toggle; click reveals one element at a time, back steps reverse) · **transition library** (fade/push/wipe/zoom per slide) · **video & audio on slides** (drag/resize like any element; playback never advances the slide) · **sticker & symbol library** (48 curated + any custom character) · **group/ungroup** (⌘G/⇧⌘G — click any member to move the group) · **smart alignment guides** (snaps to slide center/edges AND sibling edges/centers with live guide lines) · **layout templates** (Big statement / Two columns / Image+caption / Agenda) · **PNG export** (current slide) · **cloud deck storage** (save/open under your account) · **Canva-grade interactions**: marquee rubber-band selection, ⌥-drag duplicate, exact numeric text sizing, ⌘⇧>/< size shortcuts, Esc exits text editing.\n*Share-links for cloud decks need a database-rules change (public read of a shared deck) — queued for Stage 4 alongside co-editing.*
