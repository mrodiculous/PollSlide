...
/* A backup only counts if it was STORED off-site.
 *
 * api/backup.js has three outcomes and they look alike in the log:
 *   { ok: true,  file, bucket }  → written to the bucket. This is a real backup.
 *   { ok: false, error }         → tried and failed.
 *   { mode: 'download' }         → NO bucket configured, so it streamed the JSON to
 *                                  whoever asked and stored nothing. The cron cannot
 *                                  persist this. It is not a backup.
 * Counting the third as success would mask having no off-site copy at all, which is
 * the exact failure this check exists to catch. Hence `ok === true`, not `ok !== false`.
 */
function isStoredBackup(v) { return !!v && v.ok === true && !!v.at; }

function evalBackupAge(d) {
  const maxH = d.maxAgeHours || 48;

  // No destination configured beats any age question: there is nowhere for a backup
  // to go, so re-running one cannot help and must not be attempted on a loop.
  if (d.bucketConfigured === false) {
    return {
      ok: false, configIssue: true,
      detail: 'BACKUP_BUCKET is not set, so there is nowhere to store a backup. The nightly cron runs, builds the export, and discards it — you currently have no off-site copy.',
    };
  }
...
/* Pending shares hold a full deck snapshot. Nothing expires them today, so a share
 * sent to a mistyped address keeps a copy of someone's quiz — answers included —
 * indefinitely. Accepted ones don't need their payload any more either. Decks get a
 * 30-day trash TTL; shares got nothing, which is the gap this closes. */
function evalShareHygiene(d) {
  const stale = d.stalePending || 0;
  const carrying = d.settledWithPayload || 0;
  if (!stale && !carrying) return { ok: true, detail: `${d.total || 0} share record(s), nothing to tidy.` };
  const bits = [];
  if (stale) bits.push(`${stale} pending share${stale === 1 ? '' : 's'} older than ${d.maxAgeDays || 30} days`);
  if (carrying) bits.push(`${carrying} settled share${carrying === 1 ? '' : 's'} still carrying a deck snapshot`);
  return { ok: false, detail: bits.join(' and ') + '.' };
}

/* Decks nobody has opened since stable question ids shipped still key their answers
 * by array position — so reordering one silently mis-attributes its responses. The
 * browser backfills on open; this catches the ones nobody has opened. */
function evalQidBackfill(d) {
  const n = d.decksNeedingIds || 0;
  if (!n) return { ok: true, detail: `All ${d.decksChecked || 0} deck(s) have stable question ids.` };
  return { ok: false, detail: `${n} deck(s) still key answers by question position — reordering one would mis-attribute its responses.` };
}

/* A grant pointing at a deck that no longer exists. Harmless to the collaborator
 * (they just see nothing) but it accumulates, and it makes the admin view lie about
 * who has access to what. */
function evalOrphanGrants(d) {
  const n = d.orphans || 0;
  if (!n) return { ok: true, detail: `${d.grantsChecked || 0} grant(s), all pointing at live decks.` };
  return { ok: false, detail: `${n} collaboration grant(s) point at decks that no longer exist.` };
}
...
module.exports = {
  ESCALATE_AFTER_MS, TIER_RANK, isStoredBackup,
